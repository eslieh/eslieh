# 🚀 Eslieh Victor – Software Engineer

Building fast, beautiful, and reliable apps for web & mobile.  
I help startups and businesses bring their ideas to life using **React Native**, **React**, and **Flask** — from MVP to production.

[![Portfolio](https://img.shields.io/badge/🌐_View_My_Work-blue)](#projects)  
[![Contact](https://img.shields.io/badge/📩_Get_in_Touch-green)](#contact)

---

## 👨‍💻 About Me
Hi! I’m **Eslieh**, a Kenyan software engineer passionate about creating products that work seamlessly and look great.  
I specialize in building fast, scalable web and mobile applications using **React**, **React Native**, and **Flask**.

- 🚀 5+ projects launched  
- 🛠 React Native, React, Flask, Python, PHP  
- 🌍 Worked with clients from Kenya & internationally  
- 🎯 Focus: speed, performance, and beautiful UI  

---

## 🛠 Tech Stack

**Frontend:** React, React Native, JavaScript, HTML, CSS  
**Backend:** Flask, Python, PHP, REST APIs  
**Tools & Platforms:** Git, Docker, Firebase, AWS

---

## 📂 Projects

### 1. DuoTasks – *Uber for Everyday Tasks*
A platform connecting people with tasks to people with skills in African cities.

**Tech:** React Native, Flask, PostgreSQL  
**Role:** Full-stack developer — built the API, mobile app UI, and real-time task matching system.  
[🔗 Live Demo](#) | [💻 GitHub Repo](#)

---

### 2. HeartFramed – *Premium Portrait Printing*
E-commerce platform for custom wall art with free delivery across Kenya.

**Tech:** React (Vite), Flask, Stripe API  
**Role:** Frontend & backend — implemented ordering system, payments, and image processing.  
[🔗 Live Demo](#) | [💻 GitHub Repo](#)

---

### 3. Blubbb – *Voice chats for small groups*
 real-time audio chat platform where users can join virtual rooms.

**Tech:** React, Flask, WebSockets, Google OAuth  
**Role:** Full-stack developer — built authentication, live audio streaming, and chat features.  
[🔗 Live Demo](#) | [💻 GitHub Repo](#)

---

## 💬 Testimonials

> "Eslieh delivered our MVP ahead of schedule and exceeded our expectations. We’re now onboarding our first 500 users!"  
> — Client Name

---

## 📩 Contact

**Let’s Build Something Together**  
Whether you have an idea for a new app or need help improving your existing product, I’m here to help.

📧 **Email:** [esliehh@gmail.com](mailto:esliehh@gmail.com)  
💼 **LinkedIn:** [linkedin.com/in/yourprofile](#)  
💻 **GitHub:** [github.com/eslieh](#)

---

## © Footer
© 2025 Eslieh Victor — Software Engineer
